...
class FormSubGTPPage extends Page {
  ...
  async tambahApprover(index) {
    await this.inpApprover.scrollIntoView();
    await this.btnTambahKurangApprover[index].click();
  }
  async kurangApprover(index) {
    await this.btnTambahKurangApprover[index].click();
    await browser.pause(3000);
  }
