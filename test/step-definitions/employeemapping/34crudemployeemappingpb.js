...
allureReporter.addTestId("[PRINCIPAL] 34. CRUD Employee Mapping (Pojok Bayar)");
allureReporter.addFeature("CRUD Employee Mapping (Pojok Bayar)");
...
it('I input form with metode pembayaran 'Pojok Bayar' and submit', async () => {
    await frameForm();
    ...
    await FormEmployeeMappingPage.metodePB("Saldo Pojok Bayar");
...
it('System should be display employee data based on search keyword', async () => {
    await EmployeeMappingPage.expectNewEmployeeRow1(
      ...
      dataTest.employeeMapping.metodePB
    );
  }
);
